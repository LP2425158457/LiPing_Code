...
public class BookBookFrame extends Frame
  implements ActionListener
{
  ...
  private Label l1 = new Label("预约图书", 1);
  private Label l2 = new Label("请输入您要预约的图书书号: ");
  private Label l3 = new Label("您要预约的图书的详细信息如下: ");
  ...
  private Button b3 = new Button("确定预约");
  ...
  BookBookFrame(String title, Frame f2) {
    super(title);
    this.f2 = f2;
  ...
  public void actionPerformed(ActionEvent e) {
    if ((e.getSource() == this.tf) || (e.getSource() == this.b1)) {
      ...
      if (this.bt.getHead() != null) {
        ...
        new ConfirmDialog(this.f, "抱歉!预约失败!原因: 库存为空!");
      }
      ...
      if (this.r.location.bookey[this.r.index].remainder == 0) {
        ...
        new ConfirmDialog(this.f, "预约失败! 原因: 现存量足够,不需要预约!");
      }
      ...
      if (!this.tf1.getText().equals("")) {
        ...
        if (lsu.SearchUser(this.tf1.getText())) {
          this.bt.BookBookByNum(this.fInfo, this.r, this.tf1.getText());
  ...
  private class LendWindowCloser extends WindowAdapter {
    ...
    public void windowClosing(WindowEvent we) {
      if (we.getSource() == BookBookFrame.this.f) {
        BookBookFrame.this.f.setVisible(false);
        BookBookFrame.this.f2.setEnabled(true);
        BookBookFrame.this.f2.setVisible(true);
      } else if (we.getSource() == BookBookFrame.this.fInfo) {
        BookBookFrame.this.fInfo.setVisible(false);
        BookBookFrame.this.f.setEnabled(true);
        BookBookFrame.this.f.setVisible(true);
      }
    }
  }
}